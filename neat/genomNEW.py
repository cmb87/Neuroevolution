...
class Genom:
    ...
    ### Constructor ###
    def __init__(self, nids_input, nids_output, structure, maxtimelevel=2,
    ...
    def json(self):
        ...
        return {"_id": self._id, "structure": self.structure, "maxtimelevel":self.maxtimelevel,
    ...
    @property
    def nidsHidden(self):
        return [nid for nid in list(self.structure.keys()) if not nid in self.nids_input+self.nids_output]
    ...
    def initializeRandomly(cls, ninputs, noutputs, maxtimelevel=2, paddcon=0.8, paddnode=0.00, paddbias=0.01, pmutact=0.0, nrerun=None, output_activation=None):
        ...
        structure= {}
        for nid,act in zip(nids_input+nids_output, input_activation+output_activation):
            structure[nid] = {'connections':{"snids":[], "weights":[], "level":[], "innovations": [], "active":[]},'activation': act, 'bias': 0.0}

        ### Create genom ###
        genom = cls(nids_input, nids_output, structure, maxtimelevel=maxtimelevel,
    ...
    def mutate_activation(cls, genom1, generation=None):
        ...
        nid_mut = np.random.choice(genom1.nidsHidden,1)
        structure[nid_mut]["activation"] = np.random.randint(0,len(ACTIVATIONS))

        print("changed activation!!!!")
        return cls(structure=structure, nids_output=genom1.nids_output, nids_input=genom1.nids_input,
    ...
    def mutate_bias(cls, genom1, valueabs=1.0, pbigChange=0.1, generation=None):
        structure = copy.deepcopy(genom1.structure)
        nid_mut = np.random.choice(genom1.nidsHidden+genom1.nids_output,1)
    ...
    def mutate_add_connection(cls, genom1, valueabs=1, pbigChange=0.1, maxretries=60, generation=None, timelevel=None):
        ...
        nid_add = np.random.choice(genom1.nidsHidden+genom1.nids_output, 1)
